...
//                requestCode, intent, FLAG);
        ...
        cBuilder.setLargeIcon(BitmapFactory.decodeResource(mContext.getResources(), smallIcon));
...
//                requestCode, intent, FLAG);
    ...
    private void sent() {
        ...
        nm.notify(NOTIFICATION_ID, notification);
        notificationNum++;
        ShortcutBadger.applyCount(mContext, notificationNum);
    }
    ...
    public static boolean isNotificationEnabled(final Context context) {
     ...
     /* Context.APP_OPS_MANAGER */
            ...
            boolean enableNotify = ((Integer) checkOpNoThrowMethod.invoke(mAppOps, value, uid, pkg) == AppOpsManager.MODE_ALLOWED);
            if (!enableNotify) {
                DialogUtil.simpleDialog(context, context.getString(R.string.no_notification_tip), context.getString(R.string.action_settings), context.getString(R.string.button_cancel), new DialogUtil.ConfirmCallBackInf() {
                    @Override
                    public void onConfirmClick(String content) {
                        NotifyUtil.goToSetNotify(context);
                    }
                });
            }
            return enableNotify;
