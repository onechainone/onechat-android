...
import oneapp.onechat.oneandroid.onemessage.beanchat.chat.ItemAtUserMessageBody;
...
public class AtMsgEditText extends EmojiconEditText {


    public AtMsgEditText(Context context) {
        super(context);
    }

    public AtMsgEditText(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public AtMsgEditText(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public String getUserIdString() {
        ...
        for (MyTextSpan myTextSpan : spans) {
            ...
            if (realText.equals(showText)) {
                userIdList.add(myTextSpan.getUserId());
            }
        }
        if (userIdList.size() > 0) {
            ItemAtUserMessageBody itemAtUserMessageBody = new ItemAtUserMessageBody(userIdList);
            return GsonUtils.objToJson(itemAtUserMessageBody);
        } else {
            return "";
        }
    }
