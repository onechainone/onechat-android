package onemessageui.widght.lockpattern;
...
public class LockPatternView extends View {
        ...
        for (int i = 0; i < mCells.length; i++) {
            for (int j = 0; j < mCells[i].length; j++) {
                if (mCells[i][j].getStatus() == Cell.STATE_CHECK) {
                    ...
                    canvas.drawCircle(mCells[i][j].getX(), mCells[i][j].getY(),
                            this.cellRadius / 2.5f, this.defaultPaint);
    ...
    private void initCellSize() {
        this.cellRadius = (this.width - offset * 2) / 5 / 2;
    ...
    private void initPaints() {
        ...
        defaultPaint.setColor(getResources().getColor(R.color.lock_normal_color));
        defaultPaint.setStrokeWidth(2.0f);
//		defaultPaint.setStyle(Style.STROKE);
        defaultPaint.setAntiAlias(true);

        selectPaint = new Paint();
        selectPaint.setColor(getResources().getColor(R.color.lock_correct_color));
        selectPaint.setStrokeWidth(2.0f);
        //selectPaint.setStyle(Style.STROKE);
        selectPaint.setAntiAlias(true);

        errorPaint = new Paint();
        errorPaint.setColor(getResources().getColor(R.color.lock_wrong_color));
        errorPaint.setStrokeWidth(2.0f);
        //errorPaint.setStyle(Style.STROKE);
...
//                if (LockPatternUtil.checkInRound(cell.x, cell.y, cellRadius, x, y, cellRadius / 4)) {
//                    return cell;
//                }
                // FIXME: 2017/11/21
                if (LockPatternUtil.checkInRound(cell.x, cell.y, cellRadius, x, y, touchOffset)) {
                    return cell;
                }
            }
        }
    ...
    /**
     * remove the post delay action_clear pattern
     */
