package onemessageui.chat.widght;
...
public class PasteEditText extends EditText {
    ...
    public boolean onTextContextMenuItem(int id) {
        if (id == android.R.id.paste) {
            ...
            if (text.startsWith(CommonUtils.COPY_IMAGE)) {
                // intent.setDataAndType(Uri.fromFile(new
                // File("/sdcard/mn1.jpg")), "image/*");
                text = text.replace(CommonUtils.COPY_IMAGE, "");
                ...
                ((Activity) context).startActivityForResult(intent,
                        CommonUtils.REQUEST_CODE_COPY_AND_PASTE);
                // clip.setText("");
            }
        }
    ...
    protected void onTextChanged(CharSequence text, int start,
        ...
        if (!TextUtils.isEmpty(text)
                && text.toString().startsWith(CommonUtils.COPY_IMAGE)) {
            setText("");
        }
