...
public class CallActivity extends BaseActivity {
    ...
    protected void saveCallRecord(int type) {
        ...
        ItemTextItemMessageBody txtBody = null;
        if (!isInComingCall) { // 打出去的通话
            message = ItemMessage.createSendMessage(ItemMessage.Type.TXT);
            message.setReceipt(username);
        } else {
            message = ItemMessage.createReceiveMessage(ItemMessage.Type.TXT);
        ...
        switch (callingState) {
            case NORMAL:
                txtBody = new ItemTextItemMessageBody(st1 + callDruationText);
                break;
            case REFUESD:
                txtBody = new ItemTextItemMessageBody(st2);
                break;
            case BEREFUESD:
                txtBody = new ItemTextItemMessageBody(st3);
                break;
            case OFFLINE:
                txtBody = new ItemTextItemMessageBody(st4);
                break;
            case BUSY:
                txtBody = new ItemTextItemMessageBody(st5);
                break;
            case NORESPONSE:
                txtBody = new ItemTextItemMessageBody(st6);
                break;
            case UNANSWERED:
                txtBody = new ItemTextItemMessageBody(st7);
                break;
            default:
                txtBody = new ItemTextItemMessageBody(st8);
                break;
        }
        // 设置扩展属性
        if (type == 0)
        ...
        // FIXME: 2017/8/1
//		// 保存
//		getInstance().saveMessage(message, false);
    }
